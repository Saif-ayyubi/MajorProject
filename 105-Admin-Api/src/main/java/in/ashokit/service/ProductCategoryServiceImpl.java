...
public class ProductCategoryServiceImpl implements ProductCategoryService{
    ...
    public ProductCategoryDto createCategory(ProductCategoryDto categoryDto) {
        ProductCategoryEntity byCategoryNameIgnoreCase = categoryRepo.findByCategoryNameIgnoreCase(categoryDto.getCategoryName());//to check record available or not before creating or inserting//retrieving the record
        if (byCategoryNameIgnoreCase != null) {//if record available we should not create duplicate record
